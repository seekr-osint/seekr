...
func PrintTypeTreeRec(t reflect.Type, visited map[reflect.Type]bool, indentLevel int, initialIndentLevel int) string {
	...
	case reflect.Struct:
		...
		for i := 0; i < t.NumField(); i++ {
			...
			case reflect.Struct:
				sb.WriteString(fmt.Sprintf("%s  %s\t%s", strings.Repeat(" ", indentLevel+2), field.Name, PrintTypeTreeRec(field.Type, visited, indentLevel+4, initialIndentLevel)))
			case reflect.Map:
					sb.WriteString(fmt.Sprintf("%s%s\t%s[%s]%s", strings.Repeat(" ", indentLevel+4), field.Name, ColorType(field.Type.Kind()), ColorType(field.Type.Key().Kind()), PrintTypeTreeRec(field.Type.Elem(), visited, indentLevel+4, 0)))
			case reflect.Slice:
					sb.WriteString(fmt.Sprintf("%s%s\t[]%s", strings.Repeat(" ", indentLevel+4), field.Name, PrintTypeTreeRec(field.Type.Elem(), visited, indentLevel+4, 0)))
			default:
				sb.WriteString(fmt.Sprintf("%s  %s\t\t%s\n", strings.Repeat(" ", indentLevel+2), field.Name, PrintTypeTreeRec(field.Type, visited, indentLevel+4, 0)))
		...
		for i := 0; i < t.NumField(); i++ {
			field := t.Field(i)
			if field.Anonymous {
				sb.WriteString(PrintTypeTreeRec(field.Type, visited, indentLevel+4, initialIndentLevel))
			}
		}
	...
	case reflect.Map:
			sb.WriteString(fmt.Sprintf("%s%s[%s]%s\n", strings.Repeat(" ", initialIndentLevel), ColorType(t.Kind()), ColorType(t.Key().Kind()), PrintTypeTreeRec(t.Elem(), visited, indentLevel, 0)))
	case reflect.Slice:
			sb.WriteString(fmt.Sprintf("%s[]%s", strings.Repeat(" ", initialIndentLevel), PrintTypeTreeRec(t.Elem(), visited, indentLevel, 0)))
	default:
		sb.WriteString(ColorType(t.Kind()))
